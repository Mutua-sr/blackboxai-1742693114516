...
const COUCHDB_URL = 'https://couchdb.blackbox.ai'; // Replace with the actual remote CouchDB URL
    ...
    try {
      ...
      const trendingTopics = Array.from(tagCounts.entries())
        .map(([tag, count]) => ({
          tag,
          count,
          trending: count > 5, // Consider topics with more than 5 posts as trending
        }))
